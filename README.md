...
searchNextParenthesis
Funcion que busca el próximo paréntesis a resaltar recursivamente. Si encuentra un paréntesis opuesto al buscado incrementa el flag, si encuentra el paréntesis y el flag es 0 retorna la posición. Si el flag es mayor, se invoca nuevamente con el flag - 1. Si se llega al final del archivo retorna Nothing.
```haskell
...
highlightChar
Fución que aplica un tag a un caracter. 
```haskell
