...
En caso de que se vuelva a presionar el botón, se vuelve a mostrar la definición de la función y el botón vuelve a su estado anterior (no oscurecido para indicar que no esta activada esta funcionalidad).
            ...
            do
                ...
                case prevop of
                        ...
                        _ -> do
                                ...
                                nextiter <- textBufferGetIterAtOffset buffer 
(nextiteroffset + 1)
                                nextclo <- searchNextParenthesis buffer ')' 
(1) nextiter 0
                                highlightParenthesis buffer prevop nextclo 
parenthesisTag
```
...
```
Primero se obtiene el clipboard del sistema operativo, luego se obtiene el texto que se encuentra en dicho clipboard y por último se lo copia en el buffer de la ventana de edición de texto empleando la función "pasteCallBack"; esta última se emplea en forma asincrónica a través de la función "clipboardRequestText".
...
El quinto parámetro que recibe el constructor de un "HsModule" es de tipo "[HsDecl]"; esto último representa un arreglo de declaraciones de Haskell. Una declaración de Haskell (valor tipo "HsDecl") tiene los siguientes constructores entre otros : "HsTypeSig","HsDataDecl", o "HsFunBind". Estos tipos de declaraciones de Haskell son procesados por la función "process_hsdecl", la cual delega el procesado de cada tipo constructor a funciones específicas para cada caso.
...
processHsMatch xs (y:ys) buffer table = do
					...
					--se crea el botón para colapsar el código de dicha función
...
El comienzo de la siguiente declaración se extrae del valor tipo SrcLoc que contine una declaración ( HsDecl). Esto se encuentra implementado en las funciones getStartIter y getStartIterRec:
...
+ un iterador que apunta al final de la palabra actual.
...
--como resultado del parseo se obtiene un módulo. 
--El parámetro "[hsDecl]" es un arreglo de declaraciones. 
--Las declaraciones son definiciones de datos, de funciones, signature de función, etc.
...
--HsMatch representa una definición de función con sus respectivos parámetros.
--SrcLoc contiene linea y columna en el archivo. Hsname el nombre de la función. 
--HsPat son los argumentos que recibe la definicion.
